class ModelosController < ApplicationController
  ...
  # GET /modelos.json
  def index
    @modelos = Modelo.order("name_model").page(params[:page]).per_page(5)
  end
  ...
  def create
    ...
    respond_to do |format|
      if @modelo.save
        format.html { redirect_to '/modelos', notice: 'Registro guardado' }
  ...
  def update
    respond_to do |format|
      if @modelo.update(modelo_params)
        format.html { redirect_to '/modelos', notice: 'Registro Actualizado' }
  ...
  private
    ...
    def modelo_params
      params.require(:modelo).permit(:name_model)
    end
end
